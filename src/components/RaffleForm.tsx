...
const RaffleForm = () => {
  ...
  const selectedCanOption = canOptions.find(option => option.label === formData.cansQuantity);
  ...
  return <form onSubmit={handleSubmit} className="space-y-8 md:space-y-8 w-full form-mobile">
      <div className="space-y-4 md:space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <Label htmlFor="cultures" className="flex items-center gap-3 min-h-[44px] group px-2 py-2 rounded-lg hover:bg-gold/5 transition-colors duration-200 cursor-pointer">
              <Checkbox id="cultures" checked={formData.reasons.includes("cultures")} onCheckedChange={checked => {
              if (checked) {
                setFormData({
                  ...formData,
                  reasons: [...formData.reasons, "cultures"]
                });
              } else {
                setFormData({
                  ...formData,
                  reasons: formData.reasons.filter(r => r !== "cultures")
                });
              }
            ...
            <Label htmlFor="jewish" className="flex items-center gap-3 min-h-[44px] group px-2 py-2 rounded-lg hover:bg-gold/5 transition-colors duration-200 cursor-pointer">
              <Checkbox id="jewish" checked={formData.reasons.includes("jewish")} onCheckedChange={checked => {
              if (checked) {
                setFormData({
                  ...formData,
                  reasons: [...formData.reasons, "jewish"]
                });
              } else {
                setFormData({
                  ...formData,
                  reasons: formData.reasons.filter(r => r !== "jewish")
                });
              }
            }} className="border-gold/60 text-gold data-[state=checked]:bg-gold data-[state=checked]:border-gold focus-visible:ring-gold/40" />
              <span className="text-base font-normal text-foreground/90 group-hover:text-gold transition-colors duration-200 leading-relaxed">
            ...
            <Label htmlFor="support" className="flex items-center gap-3 min-h-[44px] group px-2 py-2 rounded-lg hover:bg-gold/5 transition-colors duration-200 cursor-pointer">
              <Checkbox id="support" checked={formData.reasons.includes("support")} onCheckedChange={checked => {
              if (checked) {
                setFormData({
                  ...formData,
                  reasons: [...formData.reasons, "support"]
                });
              } else {
                setFormData({
                  ...formData,
                  reasons: formData.reasons.filter(r => r !== "support")
                });
              }
            }} className="border-gold/60 text-gold data-[state=checked]:bg-gold data-[state=checked]:border-gold focus-visible:ring-gold/40" />
              <span className="text-base font-normal text-foreground/90 group-hover:text-gold transition-colors duration-200 leading-relaxed">
            ...
            <Label htmlFor="other-reason" className="flex items-center gap-3 min-h-[44px] group px-2 py-2 rounded-lg hover:bg-gold/5 transition-colors duration-200 cursor-pointer">
              <Checkbox id="other-reason" checked={formData.reasons.includes("other")} onCheckedChange={checked => {
              if (checked) {
                setFormData({
                  ...formData,
                  reasons: [...formData.reasons, "other"]
                });
              } else {
                setFormData({
                  ...formData,
                  reasons: formData.reasons.filter(r => r !== "other"),
                  otherReason: ""
                });
              }
            }} className="border-gold/60 text-gold data-[state=checked]:bg-gold data-[state=checked]:border-gold focus-visible:ring-gold/40" aria-controls="other-reason-textarea" />
              <span className="text-base font-normal text-foreground/90 group-hover:text-gold transition-colors duration-200 leading-relaxed">
                Other
              </span>
            </Label>
          ...
          {formData.reasons.includes("other") && <div className="space-y-2 mt-2 pl-8 animate-fade-in">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            <div className="relative">
              <Label htmlFor="cansQuantity" className="text-foreground font-bold text-lg md:text-xl block relative pb-2">
            ...
            <Select value={formData.cansQuantity} onValueChange={value => setFormData({
            ...formData,
            cansQuantity: value
          })}>
              <SelectTrigger id="cansQuantity" aria-label="Select quantity of cans" className="bg-input/80 backdrop-blur-sm border-border/60 text-foreground placeholder:text-foreground/50 focus:border-gold focus:ring-2 focus:ring-gold/40 transition-all duration-300 hover:border-gold/60 hover:shadow-[0_0_15px_rgba(255,215,0,0.2)]">
              ...
              <SelectContent className="bg-card/95 backdrop-blur-md border-border/60 text-foreground shadow-lg mobile-select-content">
                {canOptions.map(option => <SelectItem key={option.quantity} value={option.label} className="text-foreground focus:bg-gold/10 focus:text-gold hover:bg-gold/5 cursor-pointer transition-colors">
                    {option.label}
                  </SelectItem>)}
              </SelectContent>
            </Select>
            ...
            <input type="hidden" name="cans_quantity" value={cansQuantity} />
            <input type="hidden" name="cans_amount_usd" value={cansAmountUsd.toFixed(2)} />
          </div>
          ...
          <div className="relative z-10 space-y-3">
            ...
            <div className="space-y-2 text-sm text-foreground/80 leading-relaxed">
              ...
              <ol className="list-decimal list-inside space-y-1.5 ml-2">
                <li>Drop off cans at the Chabad Westville.</li>
        ...
        <div className="space-y-2">
          ...
          <Textarea id="comments" name="comments" placeholder="Share your thoughts or any special requests…" value={formData.comments} onChange={e => setFormData({
          ...formData,
      ...
      <div className="pt-4">
        <Button type="submit" disabled={isSubmitting} className="w-full relative overflow-hidden bg-gradient-to-r from-gold via-amber to-gold text-background font-semibold text-lg py-6 rounded-xl shadow-lg hover:shadow-[0_0_40px_rgba(255,215,0,0.6)] transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] border border-gold/30 group disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100">
          <span className="relative z-10">
            {isSubmitting ? "Processing..." : formData.sponsorships.length > 0 || formData.cansQuantity !== "" ? "Pay Now" : "Submit Entry"}
          </span>
